 ...
 */
 ...
 int equalStacks(int h1_count, int* h1, int h2_count, int* h2, int h3_count, int* h3) {
    ...
    // Initialize indices for each stack
    int i = 0, j = 0, k = 0;

    // Reduce stacks until all heights are equal
    while (i < h1_count && j < h2_count && k < h3_count) {
        if (height1 == height2 && height2 == height3) {
            return height1; // Found the maximum equal height
        }
    ...
    return 0; // If any stack becomes empty, return 0
}
*/
int equalStacks(int h1_count, int *h1, int h2_count, int *h2, int h3_count, int *h3)
{
